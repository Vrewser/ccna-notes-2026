...
---
## RIP ROUTING vs OSPF
* RIP ROUTING is good for small topologies / business networks due to simplicity and less hassle in configuration.
* OSPF is good for larger topologies like enterprises due to complex setup required for broader networks and smarter routing as each connection are further from one another.
...
### SPF Tree
* This is a loop-free graphical map of the shortest paths from a single router to all other routers/destination inside the network. So instead of hopping from one router to another if the destination is not in that hopped-router it will hop to another to find it, the SPF tree calculates and paves a way for the router to find their destination inside the network without blindly hopping from one router to another.
